...
//    }
    ...
    @Override
    public boolean hasPrivilege(String sessionId, String actionUrl) {
        if (oConvertUtils.isNotEmpty(sessionId) && oConvertUtils.isNotEmpty(actionUrl)) {
            Long authSize = Long.valueOf(0);
            Session session = sessionRepository.getSession(sessionId.trim());
            String rolecode = null;
            if (session != null) {
                Authentication authentication = (Authentication)session.getAttribute(sessionId.trim());
            ...
            if (oConvertUtils.isNotEmpty(rolecode)) {
                String sql = "SELECT count(*) FROM t_s_function f,t_s_role_function  rf,t_s_role ru " +
                        ...
                        "ru.rolecode='" + rolecode + "' AND f.functionurl = '" + actionUrl.trim() + "'";
